...
public class MixerCupAreaScript : MonoBehaviour
{
    ...
    public bool mixerIsMixing = false;
    ...
    void Update()
    {
        if(!mixerIsMixing){
            ...
            //If a possible future boba drink is rendered in this mixer, show that it's done. Else, check if it's an empty cup!
            ...
            //Check if a FINISHED MIXED boba drink is in the mixer.
            if (gameObject.transform.childCount != 0)
            {
                if (gameObject.transform.GetChild(0).tag == "FinishedBobaDrink")
                {
                ...
                else if (mixerHasCupInIt && mixerCupHasBaseInIt || mixerCupHasAddOnInIt){
